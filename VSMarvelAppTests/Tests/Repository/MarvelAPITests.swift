...
class MarvelAPITests: XCTestCase {
    ...
    func test_decode_verifyContract() {
        ...
        if let path = Bundle(for: Self.self).path(forResource: "Characters", ofType: "json") {
            do {
                ...
                XCTAssertEqual(resp.results[0].id, 1009144)
            } catch {
    ...
    func test_request_infoPassed() {
        ...
        sut.getCharacters(id: nil, queries: []) { result in }
    ...
    func test_request_resultError() {
        ...
        let completion = (session.completionSpy as! ((Result<MarvelAPI.Response.DataReceived, VSessionError>) -> ()))
    ...
    func test_request_resultSuccess() {
        let session = SesionMock()
        sut = .init(session: session)
        var resultData: Result<MarvelAPI.Response.DataReceived, VSessionError>?
